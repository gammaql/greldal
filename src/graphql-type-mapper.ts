...
import { GraphQLObjectType, GraphQLFieldConfig, GraphQLFieldConfigMap, GraphQLInputObjectType, GraphQLInputFieldConfigMap, GraphQLInputFieldConfig, GraphQLString, GraphQLFloat, GraphQLList, GraphQLBoolean, GraphQLOutputType, GraphQLScalarType, GraphQLFieldMap, GraphQLInputType } from "graphql";
...
export function ioToGraphQLOutputType(type: t.Type<any>, id: string): GraphQLOutputType {
    ...
    if (type instanceof t.InterfaceType) return new GraphQLObjectType({
        name: uniqueId('GrelDALAutoDerivedOutputType'),
        fields: transform(type.props, (result: GraphQLFieldConfigMap<any, any>, val: t.Type<any>, key: string) => {
            result[key] = {
...
export function ioToGraphQLInputType(type: t.Type<any>, id: string): GraphQLInputType {
    ...
    if (type instanceof t.InterfaceType) return new GraphQLInputObjectType({
        name: uniqueId('GrelDALAutoDerivedInputType'),
        fields: transform(type.props, (result: GraphQLInputFieldConfigMap, val: t.Type<any>, key: string) => {
            result[key] = {
...
export const deriveFieldOutputType = (field: MappedField) =>
    ioToGraphQLOutputType(field.type, field.keyPath);

export const deriveFieldInputType = (field: MappedField) =>
    ioToGraphQLInputType(field.type, field.keyPath);
